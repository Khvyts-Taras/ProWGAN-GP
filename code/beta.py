...
class ResBlock2x(nn.Module):
    ...
    def _initialize_weights(self):
        ...
        nn.init.normal_(self.upsample.weight, 0, 0.02)
        nn.init.constant_(self.upsample.bias, 0)
        nn.init.normal_(self.conv2.weight, 0, 0.02)
        nn.init.constant_(self.conv2.bias, 0)

    def forward(self, x):
        residual = F.interpolate(x, scale_factor=2, mode='nearest')  # Масштабирование для соответствия размеров
...
class FullModel(nn.Module):
    ...
    def train_model(self, data_loader, num_epochs, num_resblocks, n_critic, gradient_penalty_weight):
        ...
        for block_index in range(num_resblocks + 1):
            ...
            if block_index == 0:
                optimizer_g = Adam(self.generator.parameters(), lr=0.0002, betas=(0.5, 0.999))
                current_block = self.generator
            else:
                optimizer_g = Adam(self.resblocks[block_index - 1].parameters(), lr=0.0002, betas=(0.5, 0.999))
            ...
            for epoch in range(num_epochs):
                for i, (imgs, _) in enumerate(data_loader):
                    ...
                    fake_imgs = F.interpolate(fake_imgs, size=128, mode='nearest')
                    ...
                    if total_iters % n_critic == 0 or i == 0:
                        ...
                        gen_imgs = F.interpolate(gen_imgs, size=128, mode='nearest')
                    ...
                    if i % 100 == 0:
                        print(f"Block [{block_index}/{num_resblocks}], Epoch [{epoch}/{num_epochs}], Step [{i}]  Loss D: {d_loss.item()}, Loss G: {g_loss.item()}")
...
n_critic = 3  # Количество итераций обучения дискрим
